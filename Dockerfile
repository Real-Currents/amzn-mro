...
RUN cd /var/task && source /var/task/setup.sh && ldconfig && \
    ...
    /var/task/bin/Rscript -e 'install.packages(c("rlang", "devtools", "geojsonsf", "geoviz", "jsonlite", "magick", "magrittr", "openxlsx", "Rcpp", "RcppRedis", "raster", "remoter", "remotes", "reticulate", "rmarkdown", "rgdal", "rgeos", "sf", "sp", "slippymath", "stringr", "tidyverse", "DT"));' && \
