...
RUN yum groupinstall -y "Development Tools" && \
    ...
    yum install -y less libcurl libcurl-devel readline-static readline-devel libcairo libpng12 libXt m4 pango pango-devel python-devel python3-pip which xz udunits2 udunits2-devel unzip zip && \
...
RUN source ~/.bashrc && cd /var/task && source /var/task/setup.sh && Rscript -e 'install.packages(c("curl", "httr")); Sys.setenv(CURL_CA_BUNDLE="/var/task/lib64/R/lib/microsoft-r-cacert.pem")' && \
