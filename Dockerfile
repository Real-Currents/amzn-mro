...
RUN yum groupinstall -y "Development Tools" && \
    ...
    yum -y update && \
    yum install -y less freeglut-devel gmp-devel gdal-devel geos-devel proj-devel proj-nad proj-epsg libcairo libcurl libcurl-devel libpng12 libXt m4 pango pango-devel python-devel python3-pip readline-static readline-devel which xz udunits2 udunits2-devel unzip zip && \
...
RUN cd /var/task && rm bin && source /var/task/setup.sh && Rscript -e 'install.packages(c("curl", "httr")); Sys.setenv(CURL_CA_BUNDLE="/var/task/lib64/R/lib/microsoft-r-cacert.pem")' && \
